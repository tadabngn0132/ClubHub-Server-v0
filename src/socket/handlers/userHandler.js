...
export const setupUserHandler = (io, socket) => {
  ...
  const markUserOnline = (userId) => {
    ...
    io.emit(SOCKET_EVENTS.USERS_ONLINE_STATUS_UPDATE, {
      onlineUsers: Array.from(onlineUsers.keys()),
    });

    console.log(
      `User ${userId} online. Total online users: ${onlineUsers.size}`,
    );
  };
  ...
  socket.on("disconnect", () => {
    ...
    if (!userIdToRemove) {
      console.warn(
        `Socket ${socket.id} disconnected but no associated userId found.`,
      );
      return;
    }
    ...
    console.log(
      `User ${userIdToRemove} offline. Total online: ${onlineUsers.size}`,
    );
  });
};
