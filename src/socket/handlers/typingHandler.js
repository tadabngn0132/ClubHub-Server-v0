...
export const setupTypingHandler = (io, socket) => {
  ...
  socket.on(SOCKET_EVENTS.USER_TYPING, async (data = {}, ack) => {
    ...
    try {
      ...
      io.to(`room_${chatRoomId}`).emit(SOCKET_EVENTS.USER_TYPING, {
        userId: senderId,
        chatRoomId,
      });
      ...
      safeAck(ack, {
        success: false,
        message: error.message || "Failed to send typing status",
      });
    }
  });
  ...
  socket.on(SOCKET_EVENTS.USER_STOP_TYPING, async (data = {}, ack) => {
    ...
    try {
      ...
      io.to(`room_${chatRoomId}`).emit(SOCKET_EVENTS.USER_STOP_TYPING, {
        userId: senderId,
        chatRoomId,
      });
      ...
      safeAck(ack, {
        success: false,
        message: error.message || "Failed to send stop typing status",
      });
    }
  });
};
