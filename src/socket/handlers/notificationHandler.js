    ...
    */
    ...
    socket.on(SOCKET_EVENTS.NOTIFICATION_SEND, async (data = {}, ack) => {
        ...
        try {
            ...
            safeAck(ack, { success: false, error: error.message });
        }
    });
    ...
    socket.on(SOCKET_EVENTS.NOTIFICATION_READ, async (data = {}, ack) => {
        ...
        try {
            ...
            if (notification.userId !== userId) {
                return safeAck(ack, { success: false, error: "Unauthorized" });
            }
            const updatedNotification = await updateNotificationService(notificationId, { isRead: true });
            io.to(`user:${userId}`).emit(SOCKET_EVENTS.NOTIFICATION_READ, updatedNotification);
    ...
    socket.on(SOCKET_EVENTS.NOTIFICATION_SOFT_DELETE, async (data = {}, ack) => {
        ...
        try {
            ...
            const deletedNotification = await softDeleteNotificationService(notificationId);
            io.to(`user:${userId}`).emit(SOCKET_EVENTS.NOTIFICATION_SOFT_DELETE, deletedNotification);
